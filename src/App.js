...
function TrainerPanel({ onWeekAdded }) {
  const [semanaName, setSemanaName] = useState("");
  const [mes, setMes] = useState("Marzo");
  const [dias, setDias] = useState([EMPTY_DIA(),EMPTY_DIA(),EMPTY_DIA()]);
  const [loading, setLoading] = useState(false);
...
Reglas: tipo num = código numérico (codigos es array). tipo libre = texto libre. series formato "X/Y Z". nota para variantes especiales.`}
          ]}]
        })
      });
      ...
      setDias(parsed.dias.map(d=>({...d,ejercicios:d.ejercicios||[]})));
      setMode("manual");
  ...
  const save = async () => {
    ...
    try {
      ...
      for (const dia of dias) {
        if (!dia.dia && !dia.ejercicios.some(e=>e.series||e.nombre)) continue;
        ...
        await supabase.from("sessions").insert({week_id:week.id,dia:dia.dia||`Día`,ejercicios});
      }
      setSuccess(true);
      setSemanaName(""); setDias([EMPTY_DIA(),EMPTY_DIA(),EMPTY_DIA()]); setMode(null);
      onWeekAdded();
  ...
  return (
    <div style={{background:C.card,border:`2px solid ${C.accent}`,borderRadius:16,padding:24,marginBottom:28}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:8}}>
        ...
        {success && <span style={{color:C.green,fontSize:13,fontWeight:700}}>✓ Semana publicada</span>}
      </div>
        ...
        <div>
          ...
          <div onClick={()=>imgRef.current.click()} style={{border:`2px dashed ${C.border}`,borderRadius:10,padding:28,textAlign:"center",cursor:"pointer",marginBottom:12}}
            ...
            {imgPreview ? <img src={imgPreview} alt="preview" style={{maxWidth:"100%",maxHeight:180,borderRadius:8,objectFit:"contain"}}/> :
              <><div style={{fontSize:36}}>📸</div><div style={{color:C.muted,fontSize:13,marginTop:6}}>Haz clic para subir imagen del plan</div></>}
        ...
        <div>
          ...
          {dias.map((dia,di)=>(
            <div key={di} style={{background:C.sub,borderRadius:10,padding:12,marginBottom:10}}>
              <div style={{marginBottom:8}}>{inp(dia.dia,v=>updateDia(di,"dia",v),`${di+1}º DIA`,"140px")}</div>
          ...
          <button onClick={save} disabled={loading}
            ...
            {loading?"Guardando...":"💾 PUBLICAR SEMANA"}
          </button>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const toggleSession = async (sessionId) => {
    ...
    if (existing) {
      ...
      await supabase.from("athlete_logs").insert({user_id:session.user.id,session_id:sessionId,completado:newVal,...prs});
    }
    loadAll();
  };
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"Inter, sans-serif"}}>
      <div style={{background:"#000",borderBottom:`2px solid ${C.accent}`,padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
        ...
        <button onClick={()=>supabase.auth.signOut()} style={{background:C.sub,border:`1px solid ${C.border}`,color:C.muted,borderRadius:8,padding:"8px 16px",fontSize:12,cursor:"pointer"}}>
          Log out
        </button>
      </div>
      ...
      <div style={{padding:"0 24px 32px"}}>
        {isTrainer && tab!=="prs" && <TrainerPanel onWeekAdded={loadAll}/>}
